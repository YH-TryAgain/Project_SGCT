...
class OCGHLCTPBAlgorithm(TraditionalAlgorithmInterface):
    """Progressive signature variant of HLCT-Base.

    The algorithm keeps HLCT-Base's terminal EPC verification discipline while
    replacing the main recursive split path with progressive collision-bit
    probing and signature-assisted high-order CBIT grouping.
    """

    def __init__(self, tags_in_field: List[Tag], **kwargs):
        super().__init__(tags_in_field, **kwargs)
        self.params = OCGHLCTPBParams(
        ...
        self.delegate = (
            OCGHLCTAlgorithm(tags_in_field, **kwargs)
            if self._should_use_small_cluster_guard(tags_in_field)
            else None
        )
        self.queue: Deque[OCGPBPNode] = deque([OCGPBPNode(tags=list(tags_in_field))])
        self.current_node: Optional[OCGPBPNode] = None
        self.current_plan = OCGPBPlan()
    ...
    def _target_d_for_node(self, node: OCGPBPNode) -> int:
    ...
    def _enqueue_low_d_groups(self, plan: OCGPBPlan, positions: List[int]) -> None:
        if not positions:
