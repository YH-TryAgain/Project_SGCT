...
def _check_value(epc: str, preStr: str, cycle_name: str, params: DRCTParams) -> int:
...
class DRCTAlgorithm(TraditionalAlgorithmInterface):
    def __init__(self, tags_in_field: List[Tag], **kwargs):
        ...
        self.stack: List[DRCTNode] = [DRCTNode(preStr="1", seDepth=0)]
    ...
    def _start_query(self, node: DRCTNode) -> None:
        ...
        self.pending_query_reader_bits = (
            CONSTANTS.READER_CMD_BASE_BITS + len(node.preStr) + max(1, node.seDepth.bit_length())
        )
        active_tags = [
            tag
        ...
        for tag in active_tags:
            prefix = tag.id[: len(node.preStr)].ljust(len(node.preStr), "0")
            if prefix >= node.preStr:
    ...
    def _process_response_cycle(self, tags: List[Tag], cycle_name: str) -> AlgorithmStepResult:
        ...
        if not tags:
            ...
            return self._step_result(
                ...
                operation_description=f"DRCT {cycle_name} idle",
            )

        self._record_responses(tags)
        check_bits_total = len(tags) * self.params.check_bits
        check_values = {
            _check_value(tag.id, self.current_node.preStr, cycle_name, self.params)
            for tag in tags
        }
        ...
        if len(check_values) == 1:
            ...
            return self._step_result(
                ...
                operation_description=f"DRCT {cycle_name} fallback collision after identical Check",
            )
        ...
        return self._step_result(
            ...
            operation_description=f"DRCT {cycle_name} Check collision",
        )
    ...
    def _commit_success(
        ...
        return self._step_result(
            ...
            operation_description=f"DRCT {cycle_name} success Answer={answer}",
        )
