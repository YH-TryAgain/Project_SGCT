...
def _fallback_arg(node: OCGNode, obs: OCGObservation, p: OCGHLCTParams) -> Dict:
...
def select_mode(node: OCGNode, obs: OCGObservation, p: OCGHLCTParams) -> Tuple[str, Dict]:
...
class OCGHLCTAlgorithm(TraditionalAlgorithmInterface):
    def __init__(self, tags_in_field: List[Tag], **kwargs):
        ...
        self.queue: List[OCGNode] = [
            OCGNode(tags=list(tags_in_field), f=self.params.f_default)
        ]
        self.current_node: Optional[OCGNode] = None
        self.current_obs: Optional[OCGObservation] = None
        ...
        self.current_split_children: List[OCGNode] = []
    ...
    def _inspect_node(self, node: OCGNode) -> OCGObservation:
        tags = self._active_tags(node.tags)
        if not tags:
            return OCGObservation(kind="idle")
        if len(tags) == 1:
            return OCGObservation(kind="singleton", tags=tags, common_prefix=tags[0].id)

        tag_ids = [tag.id for tag in tags]
        common_prefix, collision_positions = RfidUtils.get_collision_info(tag_ids)
        return OCGObservation(
            kind="collision",
            tags=tags,
    ...
    def _inspect_cost(self, obs: OCGObservation) -> AlgorithmStepResult:
        if obs.kind == "idle":
            ...
            return self._step_result(
                ...
                operation_description="HLCT-Base Inspect idle",
            )
        ...
        return self._step_result(
            ...
            operation_description="HLCT-Base Inspect collision",
        )
        ...
        if self.current_mode == "POP_NODE":
            ...
            if (
                ...
                if len(active_tags) > 1:
                    self.current_obs = OCGObservation(kind="collision", tags=active_tags)
            ...
            if (
                ...
                self.current_obs = OCGObservation(
                    kind="collision",
                    tags=active_tags,
    ...
    def _split_reader_bits(self, mode: str, arg: Dict, obs: OCGObservation) -> float:
    ...
    def _make_groups(self, mode: str, arg: Dict, node: OCGNode, obs: OCGObservation) -> List[List[Tag]]:
        if mode in {"LOCK1", "LOCK2"}:
    ...
    def _make_fallback_groups(self, obs: OCGObservation, arg: Optional[Dict] = None) -> List[List[Tag]]:
    ...
    def _execute_next_group(self) -> AlgorithmStepResult:
        ...
        if not group:
            ...
            result = self._step_result(
                ...
                expected_max_tag_bits=0,
                operation_description="HLCT-Base check-gated idle child",
            )
        else:
    ...
    def _classify_check_group(self, group: List[Tag], reader_bits: float) -> AlgorithmStepResult:
        ...
        return self._step_result(
            ...
            operation_description="HLCT-Base check collision child",
        )
    ...
    def _make_fused_cached_observation(self, group: List[Tag], fused_bits: int) -> Optional[OCGObservation]:
    ...
    def _child_constraint_len(
        self,
        group: List[Tag],
        cached_obs: Optional[OCGObservation],
    ) -> int:
        if cached_obs is not None:
    ...
    def _verify_group(
        ...
        if len(group) == 1:
            ...
            if perfect_response == noisy_response:
                ...
                return self._step_result(
                    ...
                    operation_description="HLCT-Base EPC verified singleton",
                )
        ...
        return self._step_result(
            ...
            expected_max_tag_bits=expected_bits,
            operation_description="HLCT-Base apparent singleton failed EPC verify",
        )
    ...
    def _enqueue_child(
        ...
        group: List[Tag],
        verify_failed: bool,
        cached_obs: Optional[OCGObservation] = None,
    ) -> None:
        assert self.current_node is not None
        assert self.current_stats is not None
        if not group:
